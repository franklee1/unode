...
unsigned int nStakeMinAge = 1 * 60 * 60; // 3 days
unsigned int nModifierInterval = 10 * 60; // time to elapse before new modifier is computed

int nCoinbaseMaturity = 30;  //30 blocks
...
const string strMessageMagic = "Unode Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees, int nHeight)
{
    ...
    if (pindexBest->nMoneySupply < MAX_MONEY){
        if(pindexBest->nHeight == 0) //premine 
        {
            nSubsidy = 500000 * COIN;  
        }else if(pindexBest->nHeight < 101)
        {
            nSubsidy = 0 * COIN; //Anti Instamine
...
{
    int64_t ret = blockValue * 0.8; //80% for masternodes
